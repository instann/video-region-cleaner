...
> [下载最新 Windows x64 版本](https://github.com/instann/video-region-cleaner/releases/latest)

1. 下载 `VideoRegionCleaner-*-windows-x64-pyinstaller.zip`，并**完整解压**。
2. 双击解压目录中的 `VideoRegionCleaner.exe`。不要把 EXE 单独移出目录。
...
发行包目前未进行代码签名，Windows SmartScreen 可能显示提醒。请仅从本仓库 Releases 下载，并使用同一 Release 中的 `SHA256SUMS.txt` 校验文件。
...
- **可靠导出**：流式 OpenCV TELEA 处理，保留音频且不缓存整段视频；实际探测 NVENC，失败自动回退 `libx264`。
...
应用逐帧读取视频，仅对选定矩形及其修复边界执行 TELEA 图像修复，再通过 FFmpeg 编码新视频并封装原音频。NVENC 只有在实际试编码成功时启用，否则自动使用 CPU `libx264`。若源音频不能直接封装到 MP4，则转码为 AAC。导出结束后，FFprobe 会核验分辨率、帧率、时长和音频流。

## 从源码运行

需要 Windows x64、Python 3.11 和 PowerShell：
...
- 无可用 NVENC 时使用 CPU；耗时取决于视频长度、分辨率、CPU 和选区复杂度。
